...
output "instance_alarm_names" {
  ...
  value = compact([
    ...
    try(aws_cloudwatch_metric_alarm.instance_cpu_credit_critical[0].alarm_name, null),
    try(aws_cloudwatch_metric_alarm.instance_memory_high[0].alarm_name, null),
    try(aws_cloudwatch_metric_alarm.instance_memory_available_low[0].alarm_name, null),
    try(aws_cloudwatch_metric_alarm.instance_swap_warning[0].alarm_name, null),
    try(aws_cloudwatch_metric_alarm.instance_swap_critical[0].alarm_name, null),
    try(aws_cloudwatch_metric_alarm.instance_root_filesystem_high[0].alarm_name, null),
    try(aws_cloudwatch_metric_alarm.instance_root_inodes_high[0].alarm_name, null),
  ])
}
